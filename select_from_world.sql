...
-- 11. Show the name and the capital where the first letters of each match (N.B not when they are the same)
SELECT name, capital
FROM world
WHERE LEFT(name, 1) = LEFT(capital, 1) AND name <> capital;

-- 12. Find the country that has all the vowels and no spaces in it's name
SELECT name
FROM world
WHERE name NOT LIKE '% %'
AND name LIKE '%a%'
